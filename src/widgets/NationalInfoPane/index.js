...
import widgets from '../index.js'; // Импортируем глобальный реестр виджетов

// --- БЛОК ВСПОМОГАТЕЛЬНЫХ ФУНКЦИЙ ---
// (Этот блок остается без изменений, так как он корректен)

function handlePieChartMouseMove(event, canvas, sectors, tooltipEl) {
    if (!tooltipEl) return;
    ...
    if (Math.sqrt(dx * dx + dy * dy) <= centerX) {
        ...
        for (const sector of sectors) {
            if (angle >= sector.startAngle && angle < sector.endAngle) {
                tooltipEl.innerHTML = `<strong>${sector.partyName}</strong><hr>Популярность: ${sector.popularity}%`;
...
function handlePieChartMouseLeave(tooltipEl) {
    if (tooltipEl) tooltipEl.style.display = 'none';
}

function drawPoliticalPieChart(canvas, parties) {
    ...
    filteredParties.forEach(party => {
        ...
        ctx.lineWidth = 1;
        ctx.stroke();
        sectors.push({ partyName: party.name, popularity: party.popularity, startAngle: currentAngle, endAngle });
    ...
    canvas._handleMouseMove = (event) => handlePieChartMouseMove(event, canvas, sectors, tooltipEl);
    canvas._handleMouseLeave = () => handlePieChartMouseLeave(tooltipEl);
    canvas.addEventListener('mousemove', canvas._handleMouseMove);
    canvas.addEventListener('mouseleave', canvas._handleMouseLeave);
}

function updatePartyList(listContainer, parties, rulingPartyId) {
    if (!listContainer || !parties || parties.length === 0) return;
    ...
    sortedParties.forEach(party => {
        ...
        const effects = `Популярность: ${party.popularity}%\nИдеологии: ${party.ideology_tags_rus.join(', ')}`;
        addTooltipEvents(listItem, party.name, effects, party.short_description);
...
// --- ГЛАВНАЯ ФУНКЦИЯ ВИДЖЕТА ---
export default function NationalInfoPaneWidget(props) {
  const { state, definitions, ruling_party_id } = props;
  const mainPane = document.createElement('div');
  mainPane.className = 'national-info-pane';

  // --- 1. Баннер Национального Фокуса ---
  const focusBanner = document.createElement('div');
  focusBanner.className = 'national-focus-banner';
  focusBanner.innerHTML = `
      <img src="/history/turn_images/Autumn2003.png" alt="Дерево фокусов" class="focus-bg">
      <div class="focus-title-bar">Национальные Фокусы</div>
  `;
  addTooltipEvents(focusBanner, 'Дерево Национальных Фокусов', 'Нажмите, чтобы открыть дерево фокусов и просмотреть доступные пути развития.', null);

  focusBanner.addEventListener('click', () => {
    const FocusTreeWidget = widgets['focus-tree'];
 if (FocusTreeWidget) {
    // --- НАЧАЛО ИСПРАВЛЕНИЯ ---
    // Мы берем данные не из глобального state, а из props,
    // которые были переданы в этот виджет.
    const focusTreeEl = FocusTreeWidget({
      focus_tree: definitions.national_focus_tree,
      completed_focuses: props.state.completed_focuses
    });
...
// --- 2. Национальные Духи (ЛОГИКА ОБНОВЛЕНА) ---
const spiritsContainer = document.createElement('div');
spiritsContainer.className = 'national-spirits';

if (state.active_national_spirit_ids && definitions.national_spirits) {

    // --- НАЧАЛО НОВОЙ ЛОГИКИ: РАСЧЕТ БОНУСОВ ---
    const totalDevelopmentImpulses = {
        education: 0, healthcare: 0, welfare: 0, agriculture: 0,
        industry: 0, internal_security: 0, military_might: 0, social_development: 0
    };

    // Сначала проходим по всем активным духам и суммируем их бонусы
    state.active_national_spirit_ids.forEach(spiritId => {
        const spiritData = definitions.national_spirits[spiritId];
        if (spiritData && spiritData.development_impulses) {
            for (const areaKey in spiritData.development_impulses) {
                if (totalDevelopmentImpulses.hasOwnProperty(areaKey)) {
                    totalDevelopmentImpulses[areaKey] += spiritData.development_impulses[areaKey];
                }
            }
        }
    });
    // --- КОНЕЦ НОВОЙ ЛОГИКИ: РАСЧЕТ БОНУСОВ ---

    // Теперь проходим по всем активным духам для их отрисовки
    state.active_national_spirit_ids.forEach(spiritId => {
        const spiritData = definitions.national_spirits[spiritId];
        if (spiritData) {
            ...
            // --- ИСПОЛЬЗУЕМ НАШУ МЕТКУ! ---
            // Если это агрегатор, мы генерируем его тултип на лету
            if (spiritData.is_aggregator) {
                let dynamicSummary = "Суммарные импульсы к развитию:";
                for (const areaKey in totalDevelopmentImpulses) {
                    const areaDef = definitions.development_areas[areaKey];
                    const areaName = areaDef ? areaDef.name : areaKey; // Берем красивое имя, если есть
                    const impulseValue = totalDevelopmentImpulses[areaKey];
                    if (impulseValue !== 0) { // Показываем только ненулевые
                       dynamicSummary += `\n  ${areaName}: ${impulseValue > 0 ? '+' : ''}${impulseValue}`;
                    }
                }
                tooltipEffects = dynamicSummary;
            }
            // -----------------------------
  ...
  detailsContainer.innerHTML = `
    <div class="ruling-party-info">
      <strong>${rulingParty ? rulingParty.name : 'Партия не найдена'}</strong><br>
      <small>Следующие выборы: скоро</small>
    </div>
    <div class="political-system-info-container">
        ...
        <button class="balance-btn">Баланс</button>
    </div>
    <div class="party-popularity">
        <div class="pie-chart">
            <canvas id="partyPieChartCanvas" width="110" height="110"></canvas>
        </div>
        <ul class="party-list" id="partyListContainer"></ul>
    </div>
  `;

  // --- Собираем виджет в правильном порядке ---
  mainPane.appendChild(focusBanner);
  mainPane.appendChild(spiritsContainer);
  mainPane.appendChild(detailsContainer);

  // --- Запускаем интерактивные функции ПОСЛЕ того, как все элементы добавлены в DOM ---
