...
function handleSaveChanges() {
    ...
    // Убедимся, что последние изменения баланса попали в сохранение
...
if (balanceButton) balanceButton.onclick = () => { 
    ...
    updateBalanceScale(GAME_DATA.currentState?.balance_value !== undefined ? GAME_DATA.currentState.balance_value : 0); 
};
...
const GAME_DATA = { 
    ...
    currentState: { // Структура по умолчанию для currentState
        display_leader_id: "putin_vladimir", // Пример дефолтного значения
        display_ideology_id: "sovereign_democracy",
        ruling_party_id: "united_russia",
        active_national_focus_id: "autumn_2003_h23_main",
        active_national_spirit_ids: [
            "fortress_state", "near_abroad_policy", "national_projects_infrastructure",
            "reindustrialization_opk_tech_sovereignty", "divided_society", "systemic_corruption",
            "oil_gas_rent_stabilization_fund", "sovereign_economy", "social_initiatives_family_support",
            "russian_dream_culture_identity", "soft_power_russia_abroad"
        ],
        balance_value: 0,
        advisors_selected: {},
        corporations_selected: {},
        constitutional_principles_selected_options: {},
        development_areas_state: {}
    } 
};
...
async function initializeGameData() {
    const loadPromises = ALL_DATA_FILES_TO_LOAD.map(async (filePath) => {
        ...
        if (filePath.endsWith('leaders.json') && data.options) { GAME_DATA.leaders = {}; data.options.forEach(l => { if(l.id) GAME_DATA.leaders[l.id] = l;}); }
            ...
            if (data) {
                // Глубокое слияние, чтобы не потерять вложенные структуры по умолчанию, если их нет в файле
                ...
                // Для development_areas_state нужно более глубокое слияние по каждому ключу
                ...
                console.log("Файл состояния current_state.json загружен и объединен.");
                 ...
                 console.warn(`Файл current_state.json не найден или пуст. Будут использованы значения по умолчанию для состояния.`);
            }
        }
    });
    await Promise.all(loadPromises);
    console.log("Структура GAME_DATA после загрузки:", JSON.parse(JSON.stringify(GAME_DATA))); // Для отладки
    initializeUI();
}
...
function openSidePanelForCategory(slotType, clickedSlotEl) {
    ...
    if(!sidePanelOptionsContainer || !sidePanelTitleEl || !selectionSidePanel) { return; }
    ...
    if (slotType.startsWith("advisor_")) {
        ...
        panelTitle = "Выбор Корпорации"; optionsToShow = Object.values(GAME_DATA.corporations || {}).filter(c => typeof c === 'object' && c.id); iconPreviewClass = 'corporation-icon-preview';
    }
...
function selectOptionInSidePanel(selectedOptionId, targetSlotType) {
    ...
    if (!chosenData || !clickedMainSlotElement) { return; }
    ...
    if ((principleIdForState || developmentAreaIdForState) && parentCategoryData?.icon_path && clickedMainSlotElement.classList.contains('constitutional-principle')) {
         imgSrc = parentCategoryData.icon_path;
    }
    if (mainSlotImg) {
      mainSlotImg.src = imgSrc || 'https://via.placeholder.com/80/ccc/000?text=N/A';
    ...
    if (developmentAreaIdForState && parentCategoryData && GAME_DATA.currentState.development_areas_state?.[developmentAreaIdForState]) {
        ...
        labelTextContent = `${chosenData.name_display}<br><span class="progress-text">${currentProgress}/${progressPerLevel}</span>`;
...
function drawPoliticalPieChart() { /* ... (без изменений) ... */ }
function handlePieChartMouseMove(event, canvas, sectors, centerX, centerY, radius) { /* ... (без изменений) ... */ }
function handlePieChartMouseLeave() { /* ... (без изменений) ... */ }
function updatePartyList() { /* ... (без изменений) ... */ }
...
function initializeUI() {
    ...
    const leaderDataToDisplay = GAME_DATA.leaders?.[leaderIdToDisplay];
    if (leaderDataToDisplay && leaderPortraitEl && leaderNameEl) {
        leaderPortraitEl.src = leaderDataToDisplay.portrait_path || 'https://via.placeholder.com/156x210/555/fff?text=Ldr';
        leaderNameEl.textContent = leaderDataToDisplay.name || "Неизвестный Лидер";
        addTooltipEventsToElement(leaderPortraitEl.parentElement, leaderDataToDisplay.name, leaderDataToDisplay.tooltip_summary, leaderDataToDisplay.description);
    } else {
    ...
    const ideologyDataToDisplay = GAME_DATA.ideologies?.[ideologyIdToDisplay];
    ...
    if (ideologyDataToDisplay && ideologyIconEl && ideologyNameEl && ideologyIconContainer) {
        ideologyIconEl.src = ideologyDataToDisplay.icon_path || 'https://via.placeholder.com/65x65/555/fff?text=IDL';
        ideologyNameEl.innerHTML = ideologyDataToDisplay.name_multiline || ideologyDataToDisplay.name || "Идеология";
        addTooltipEventsToElement(ideologyIconContainer, ideologyDataToDisplay.name, ideologyDataToDisplay.effects_summary, ideologyDataToDisplay.description);
    } else {
        if (ideologyNameEl) ideologyNameEl.textContent = "Идеология не выбрана";
    ...
    const rulingPartyIdToDisplay = cs.ruling_party_id || "united_russia";
    const rulingPartyDataToDisplay = GAME_DATA.parties?.[rulingPartyIdToDisplay];
    if(rulingPartyInfoStrongEl && rulingPartyDataToDisplay) rulingPartyInfoStrongEl.textContent = rulingPartyDataToDisplay.name;
    ...
    if (rulingPartyDataToDisplay && partyEmblemEl && partyEmblemContainer) {
        partyEmblemEl.src = rulingPartyDataToDisplay.icon_path || 'https://via.placeholder.com/65x65/555/fff?text=Pty';
        let partyEffectsSummary = `Популярность: ${rulingPartyDataToDisplay.popularity}%\n`; if (rulingPartyDataToDisplay.ideology_tags_rus) partyEffectsSummary += `Идеологии: ${rulingPartyDataToDisplay.ideology_tags_rus.join(', ')}`;
        addTooltipEventsToElement(partyEmblemContainer, rulingPartyDataToDisplay.name, partyEffectsSummary, rulingPartyDataToDisplay.short_description);
    } else {
    ...
    const activeFocusId = cs.active_national_focus_id || "autumn_2003_h23_main"; // Дефолт, если не задано в состоянии
    // Предполагается, что GAME_DATA.currentNationalFocus уже настроен (например, если он всегда один)
    // или его нужно выбирать из списка всех фокусов по activeFocusId
    const focusDataToDisplay = (GAME_DATA.currentNationalFocus?.id === activeFocusId) ? GAME_DATA.currentNationalFocus : null; 
    // Если у вас будет список GAME_DATA.national_focuses, то:
    // const focusDataToDisplay = GAME_DATA.national_focuses?.[activeFocusId];
    ...
    if (nationalSpiritsContainer && GAME_DATA.national_spirits) {
        ...
        const activeSpiritIds = cs.active_national_spirit_ids || []; // Берем из состояния
        ...
        activeSpiritIds.forEach(spiritId => { /* ... (как было, но с activeSpiritIds из cs) ... */ });
        // ... (агрегатор тоже как был) ...
    }
    ...
    if (principlesContainer && GAME_DATA.constitutional_principles && cs?.constitutional_principles_selected_options) {
        ...
        Object.entries(GAME_DATA.constitutional_principles).sort(([,a], [,b]) => (a.article_number || Infinity) - (b.article_number || Infinity))
            .forEach(([principleId, principleDef]) => {
                const selectedOptionId = cs.constitutional_principles_selected_options[principleId] || 
                                       (principleDef.options?.find(opt => opt.is_current) || principleDef.options?.[0])?.id; // Фолбэк, если нет в состоянии
    ...
    if (developmentContainer && GAME_DATA.development_areas && cs?.development_areas_state) {
        ...
        Object.entries(GAME_DATA.development_areas).sort(([,a], [,b]) => (a.order || Infinity) - (b.order || Infinity))
            .forEach(([areaId, areaDef]) => {
                ...
                // Фоллбэк на значения по умолчанию из definitions если в состоянии нет
                const currentLevelId = areaState.current_level_id || areaDef.levels?.[0]?.id; // Берем первый уровень по умолчанию если нет в state или areaDef
                const currentProgress = areaState.current_progress ?? (areaDef.levels?.find(l=>l.id === currentLevelId)?.default_progress || 0) ; // Нужен default_progress в JSON уровня или 0
                ...
                if (currentLevelData) {
                    ...
                    slotEl.appendChild(labelEl);
                    const effectsForTooltip = `Прогресс: ${currentProgress}/${progressPerLevel}` + (currentLevelData.effects_summary ? `\n${currentLevelData.effects_summary}` : "") + (currentLevelData.description ? `\n\n${currentLevelData.description}` : "");
                    addTooltipEventsToElement(slotEl, currentLevelData.name_display || "Уровень", effectsForTooltip, null, areaDef.name);
                    slotEl.addEventListener('click', function() { openSidePanelForCategory(this.dataset.slotType, this); });
...
if (nationalFocusBannerClickable) {
    nationalFocusBannerClickable.addEventListener('click', () => {
        ...
        // Если у вас есть массив всех фокусов GAME_DATA.allNationalFocuses
        // const focusData = GAME_DATA.allNationalFocuses?.[activeFocusIdFromState];
        // А пока у нас только один фокус в GAME_DATA.currentNationalFocus, проверяем его ID
        ...
        if (nationalFocusModal && focusModalTitle && focusModalImage && focusModalDescription && focusModalProjectsContainer && focusData) {
            ...
            if (focusData.projects && focusData.projects.length > 0) {
                focusData.projects.forEach(project => { /* ... (как было) ... */ });
            } else {
                focusModalProjectsContainer.innerHTML = '<p style="text-align:center;color:#888;width:100%; margin: 10px 0;">Нет активных проектов.</p>';
            }
            nationalFocusModal.style.display = 'flex';
        } else { 
            console.error("Элементы модального окна нац. фокуса или данные фокуса не найдены для ID:", activeFocusIdFromState); 
        }
    });
}
...
window.addEventListener('click', function(event) {
    ...
    if (selectionSidePanel && selectionSidePanel.style.display === 'flex' && !selectionSidePanel.contains(event.target) && clickedMainSlotElement && !clickedMainSlotElement.contains(event.target) && event.target !== clickedMainSlotElement && !isDescendant(clickedMainSlotElement, event.target)) {
        let isClickOnTrigger = false;
        document.querySelectorAll('[data-slot-type]').forEach(trigger => { if (trigger.contains(event.target) || trigger === event.target) isClickOnTrigger = true; });
        if (!isClickOnTrigger) selectionSidePanel.style.display = 'none';
    }
});
